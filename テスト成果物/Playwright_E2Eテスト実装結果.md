...
- 実行手段: dev preview モード（`?dev=<stepId>`）。Tauri/Python サイドカー無しでサンプル状態の本番想定 UI を描画する（app/page.tsx `shouldUseDevPreviewMode()`）。
- 実装範囲: E2E自動テストケース TC-E2E-001〜018 のうち、dev preview で決定論的に再現できるもの。
...
| プロジェクト | chromium のみ |
| webServer | `npm run dev`（cwd=apps/desktop）を自動起動/停止。`url=http://localhost:3000`、`reuseExistingServer: !CI`、timeout 120s。テスト後に dev server を自動停止 |
...
ok 1 [chromium] › e2e\desktop-shell.e2e.spec.js:17:3 › ... › TC-E2E-011 ... (2.4s)
1 passed (3.8s)
```
...
- 実装テスト（TC-E2E-011）: テスト関数タイトルに TC-ID を含み、近傍コメントに `TC / TD / TV / TA / Risk / Spec` を保持。
- 退避テスト（17件）: `未実装テストケース_E2E自動.md` に元 TD/TV/TA/Risk・退避理由・必要対応・関連質問 ID（DQ03）付きで記載。
- E2E レーン 18 件（TC-E2E-001〜018）の内訳: 実装 1・退避 17。欠落・取りこぼしなし。
