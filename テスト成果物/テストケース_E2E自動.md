...
| TC-E2E-S1-020 | STEP1優先 | TV-S1 | TA-S1 | E2E自動 | E2E | 高 | PDF以外と存在しない/アクセス不可パスは一覧へ不正反映せず復旧できること | `?e2e=step1` で Tauri open / sidecar をハーネス化 | PDF以外、存在しないPDF、アクセス不可相当PDF、正常PDF | 1. 不正パス群を選択する 2. エラー表示と一覧未混入を確認する 3. 正常PDFを再選択する | 不正パスは一覧に混入せず、再選択した正常PDFは取り込める | Playwright実行ログ・DOM評価（apps/desktop/e2e/desktop-shell.e2e.spec.js） | DQ03（導入確定） | E2Eテスト一覧_STEP1優先.md | R-STEP1 | 作成済み |
| TC-E2E-S1-021 | STEP1優先 | TV-S1 | TA-S1 | E2E自動 | E2E | 高 | 複数PDFを連続追加しても重複せず現在選択PDFを維持すること | `?e2e=step1` で Tauri open / sidecar をハーネス化 | 正常PDF2件、重複PDF1件 | 1. PDFを1件取り込む 2. 既存PDF＋別PDFを追加する 3. 別PDFを選択する 4. 重複PDFだけを再選択する | 重複は追加されず、追加順と現在選択PDFが維持される | Playwright実行ログ・DOM評価（apps/desktop/e2e/desktop-shell.e2e.spec.js） | DQ03（導入確定） | E2Eテスト一覧_STEP1優先.md | R-STEP1 | 作成済み |
| TC-E2E-S1-022 | STEP1優先 | TV-S1 | TA-S1 | E2E自動 | E2E | 高 | 取込後に1件削除・全クリア・再取込してSTEP2へ進めること | `?e2e=step1` で Tauri open / sidecar をハーネス化 | 正常PDF2件、再取込用PDF1件、出力先 | 1. PDF2件を取り込む 2. 1件削除する 3. 全クリアする 4. PDFを再取込する 5. 出力先を設定してSTEP2へ進む | 削除、全クリア、再取込後も一覧とSTEP2遷移が正しく復旧する | Playwright実行ログ・DOM評価（apps/desktop/e2e/desktop-shell.e2e.spec.js） | DQ03（導入確定） | E2Eテスト一覧_STEP1優先.md | R-STEP1 | 作成済み |
...
| TC-E2E-S1-027 | STEP1優先 | TV-S1 | TA-S1 | E2E自動 | E2E | 高 | 出力先のキャンセル・再設定・デスクトップ戻しでSTEP1完了条件が壊れないこと | `?e2e=step1` で Tauri open / desktopDir をハーネス化 | 正常PDF1件、出力先キャンセル、出力先フォルダ、デスクトップ既定値 | 1. PDFを取り込む 2. 出力先選択をキャンセルする 3. 出力先を再設定する 4. デスクトップへ戻す | 既定出力先は維持され、再設定と戻し後もSTEP2へ進める | Playwright実行ログ・DOM評価（apps/desktop/e2e/desktop-shell.e2e.spec.js） | DQ03（導入確定） | E2Eテスト一覧_STEP1優先.md | R-STEP1/R007 | 作成済み |
| TC-E2E-S1-028 | STEP1優先 | TV-S1 | TA-S1 | E2E自動 | E2E | 高 | PDF選択中に再クリックしても取込ダイアログが二重起動しないこと | `?e2e=step1` で Tauri open を遅延させる | ファイル選択待ち相当の遅延、同一ボタンの連続クリック | 1. STEP1を開く 2. PDF選択ボタンを同一描画フレームで2回クリックする 3. open:pdf 呼び出し回数と一覧を確認する | open:pdf は1回だけ呼ばれ、PDF一覧に重複やReact key警告が発生しない | Playwright実行ログ・DOM評価（apps/desktop/e2e/desktop-shell.e2e.spec.js） | DQ03（導入確定） | E2Eテスト一覧_STEP1優先.md | R-STEP1/R007 | 作成済み |
| TC-E2E-S1-029 | STEP1優先 | TV-S1 | TA-S1 | E2E自動 | E2E | 高 | PDF追加後も出力先と共通項目を維持すること | `?e2e=step1` で Tauri open / sidecar をハーネス化 | 正常PDF2件、出力先、箱No、バインダーNo | 1. PDFを取り込む 2. 出力先と共通項目を入力する 3. PDFを追加する 4. 出力先と共通項目を確認する | PDF追加後も出力先、箱No、バインダーNoが維持され、STEP2へ進める | Playwright実行ログ・DOM評価（apps/desktop/e2e/desktop-shell.e2e.spec.js） | DQ03（導入確定） | E2Eテスト一覧_STEP1優先.md | R-STEP1 | 作成済み |
